...
void Geometry::makeTriangle(unsigned int vert0, unsigned int vert1, unsigned int vert2){
    ...
    // Compute a normal
    // Done: https://www.khronos.org/opengl/wiki/Calculating_a_Surface_Normal
    float Nx = edge0.y*edge1.z - edge0.z*edge1.y;
    float Ny = edge0.z*edge1.x - edge0.x*edge1.z;
    float Nz = edge0.x*edge1.y - edge0.y*edge1.x;
    // Use calculated normals
    normals[vert0*3+0] = Nx;	normals[vert0*3+1] = Ny;	normals[vert0*3+2] = Nz;	
    normals[vert1*3+0] = Nx;	normals[vert1*3+1] = Ny;	normals[vert1*3+2] = Nz;	
	normals[vert2*3+0] = Nx;	normals[vert2*3+1] = Ny;	normals[vert2*3+2] = Nz;	
    // normals[vert0*3+0] = 0.0;	normals[vert0*3+1] = 0.0;	normals[vert0*3+2] = 1.0;	
    // normals[vert1*3+0] = 0.0;	normals[vert1*3+1] = 0.0;	normals[vert1*3+2] = 1.0;	
	// normals[vert2*3+0] = 0.0;	normals[vert2*3+1] = 0.0;	normals[vert2*3+2] = 1.0;
