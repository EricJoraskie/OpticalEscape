...
PickerBuffer::PickerBuffer(){
    ...
    // Setup shaders for the PickerBuffer Object
    std::string fboVertexShader = fboShader->LoadShader("./shaders/fboVert.glsl");
    std::string fboFragmentShader = fboShader->LoadShader("./shaders/pickFrag.glsl");
...
void PickerBuffer::Create(int width, int height){

    // Generate a PickerBuffer
...
void PickerBuffer::Update(int filter){
    // Select our PickerBuffer
    fboShader->Bind();
    // Set the uniforms in our current shader
    ...
    fboShader->setUniform1i("filter", filter);
}

// Done with our PickerBuffer
void PickerBuffer::Unbind(){
...
void PickerBuffer::setupScreenQuad(float x1,float x2, float y1, float y2){
